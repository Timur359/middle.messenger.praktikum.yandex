import {
  ...
  ModalRegistryTypes,
  ModalKey
} from "@modals/ModalRegistry";
import { IRemoveUserModalProps } from "@modals/RemoveUserModal/RemoveUserModal";

class ModalService {
  private _params: Partial<ModalRegistryTypes> = {};

  private _showModal<TKey extends ModalKey>(
    modalName: TKey,
    params?: ModalRegistryTypes[TKey]
  ) {
    ...
    document
      .querySelector("#modal-root .modal-dialog__content")!
      .append(instance.getContent()!);
  }
  ...
  getParams<TKey extends ModalKey>(
    modalName: TKey
  ): Partial<ModalRegistryTypes>[TKey] {
